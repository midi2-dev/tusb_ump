...
// ─────────────────────────────────────────────────────────────
// Wire byte packing — little-endian ↔ UMP wire order
//
// UMP words are transmitted MSB-first on USB.  On RP2040 (LE),
                                 ...
                                 UMPStreamTxCb onTx)
{
    ...
    w[0] = streamWord0(UMP_STREAM_FMT_COMPLETE, UMP_STREAM_STATUS_DEVICE_INFO,
                       0x00, 0x00);
    w[1] = packWord(0x00, cfg.manufacturerId[0],
                    cfg.manufacturerId[1], cfg.manufacturerId[2]);
    w[2] = packWord((cfg.familyId >> 8) & 0xFF, cfg.familyId & 0xFF,
                    (cfg.modelId  >> 8) & 0xFF, cfg.modelId  & 0xFF);
    w[3] = packWord(cfg.swRevision[0], cfg.swRevision[1],
                         ...
                         const UMPStreamConfig& cfg, UMPStreamTxCb onTx)
{
    ...
    w[0] = streamWord0(UMP_STREAM_FMT_COMPLETE, UMP_STREAM_STATUS_FB_INFO,
                       0x80 | (fbIdx & 0x7F), 0x00);
    w[1] = packWord(
        (cfg.fbUIHint  << 6) | (cfg.fbMidi10  << 4) | (cfg.fbDirection << 2),
        cfg.fbFirstGroup, cfg.fbNumGroups, cfg.fbMidiCIVer
    );
                                     ...
                                     UMPStreamTxCb onTx)
{
    ...
    case UMP_STREAM_STATUS_EP_DISCOVERY: {
        ...
        if (filter & UMP_FILTER_EP_INFO)    _replyEndpointInfo(itf, cfg, onTx);
        ...
        if (filter & UMP_FILTER_PRODUCT_ID) _replyProductInstanceId(itf, cfg, onTx);
        if (filter & UMP_FILTER_STREAM_CFG) _replyStreamConfig(itf, UMP_PROTO_MIDI2, 0x00, cfg, onTx);
        return "EP Discovery";
    }
