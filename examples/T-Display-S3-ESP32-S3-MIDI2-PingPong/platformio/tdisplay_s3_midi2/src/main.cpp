// ============================================================
// USB MIDI 2.0 Device — T-Display-S3  (PlatformIO)
// ============================================================
//
// Build:  pio run -e T-Display-S3-MIDI2
// Flash:  pio run -e T-Display-S3-MIDI2 -t upload
...
//
// This example demonstrates a fully compliant USB MIDI 2.0
// device with:
//   • Dual alternate settings (Alt 0 = MIDI 1.0, Alt 1 = UMP)
//   • UMP Endpoint Discovery responses (EP Info, Device ID,
//     Endpoint Name, Product Instance ID)
//   • Function Block Discovery responses (FB Info, FB Name)
//   • Stream Configuration negotiation
//   • Real-time display of the negotiation timeline
//
...
static const UMPStreamConfig streamCfg = {
    ...
    .modelId              = 0x0001,
    .swRevision           = { '0', '1', '0', '0' },
    .endpointName         = "ESP32-S3 MIDI2",        // ≤14 chars → 1 packet
    .productInstanceId    = "ESP32S3MIDI001",         // ≤14 chars → 1 packet
    .fbName               = "Group 0",
...
// Called by umpStreamHandleRx() after each response is sent.
...
static void processRxUMP()
{
    for (int pass = 0; pass < 16; pass++) {
        ...
        if (mt == 0xF) {
            // MT=0xF Stream messages — handle with the Discovery protocol.
            // Do NOT echo these back; the stream handler sends proper responses.
